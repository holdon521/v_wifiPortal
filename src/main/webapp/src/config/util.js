 ...
 * ios系统弹框是否为ios11
 */
export const isTanIos11 = () =>{
	var str= navigator.userAgent.toLowerCase();
	var ver=str.match(/cpu iphone os (.*?) like mac os/);
	if(ver && parseInt(ver[1])>=11){
		if(str.indexOf("safari")=="-1" && str.indexOf("browser")=="-1"){
			return true;
		}
	}
	return false;
...
/**
 * 格式化当前时间 20170502102317
 */
export const SignTime = () => {
	var d = new Date();
	var en = d.getFullYear() + "-" + (d.getMonth() + 1) + "-" + d.getDate();
	var en1 = buLing(d.getHours()) + "" + buLing(d.getMinutes()) + "" + buLing(d.getSeconds());
...
export const encodePwd = (data, key) => {
	var keyHex = CryptoJS.enc.Utf8.parse(key==undefined?CONSTANTS.aesKey:key);
